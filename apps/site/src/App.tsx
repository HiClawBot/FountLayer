  ...
  en: {
    ...
    docs: {
      articles: [
        ...
        {
          body: "A trusted application backend issues a five-minute, single-use ticket binding app, channel, end user, use case, and managed mode. The browser cannot invent attribution.",
          code: [
            ...
            "});",
            "",
            'const { ticket } = await fetch("/api/session-ticket", {',
            '  method: "POST",',
        ...
        {
          body: "Run the runtime smoke check after the gateway, console, database, and managed LiteLLM adapter are online. All ports stay inside the project policy range.",
          code: [
    ...
    scenarios: [
      ...
      {
        channel: "Post-beta research",
        description:
          "Local and BYOK helpers exist as configuration foundations, but they are not active execution modes in the external beta.",
        event: {
        ...
        name: "Local / BYOK — experimental",
        outcome: "Unavailable in the supported beta data plane.",
        steps: [
          {
            detail: "SDK helpers can store configuration on the device.",
            label: "Foundation",
            metric: "storage only",
          },
          {
            detail:
              "The stock browser request path does not consume those settings.",
            label: "Execution",
            metric: "unavailable",
          },
          {
            detail:
              "External beta users remain on managed mode through LiteLLM.",
            label: "Beta mode",
            metric: "managed",
          },
          {
            detail:
              "Provider keys remain outside SDK, frontend, logs, and Git history.",
            label: "Key boundary",
            metric: "server only",
          },
          {
            detail: "Local/BYOK UI returns only after end-to-end tests pass.",
            label: "Release gate",
            metric: "deferred",
          },
        ],
  ...
  zh: {
    ...
    docs: {
      articles: [
        ...
        {
          body: "可信应用后端签发五分钟、单次使用的票据，绑定应用、渠道、终端用户、用途与 managed 模式；浏览器无法自行编造归因。",
          code: [
            'import { createFountLayer } from "@fountlayer/sdk-js";',
            "",
            "const ai = createFountLayer({",
        ...
        {
          body: "网关、控制台、数据库和 LiteLLM 管理适配器在线后运行运行时冒烟检查。所有端口保持在项目端口范围内。",
          code: [
            "GATEWAY_BASE_URL=http://localhost:3300 \\",
            "CONSOLE_BASE_URL=http://localhost:3301 \\",
            "CONSOLE_GATEWAY_ADMIN_TOKEN=change_me_admin_token \\",
    ...
    scenarios: [
      ...
      },
      {
        channel: "后续版本研究",
        description:
          "本地和自带密钥辅助函数目前只是配置基础，不属于外部测试版的有效执行模式。",
        event: {
          appId: "私有研究桌面",
          mode: "实验性",
          retail: "暂不可用",
        ...
        name: "本地 / 自带密钥 — 实验性",
        outcome: "不属于受支持的测试版数据链路。",
        steps: [
          {
            detail: "开发包辅助函数可以把配置保存在设备上。",
            label: "基础能力",
            metric: "仅存储",
          },
          {
            detail: "标准浏览器请求链路目前不会使用这些设置。",
            label: "执行链路",
            metric: "暂不可用",
          },
          {
            detail: "外部测试版统一使用 LiteLLM 管理模式。",
            label: "测试版模式",
            metric: "管理模式",
          },
          {
            detail: "服务商密钥不会进入开发包、前端、日志或代码历史。",
            label: "密钥边界",
            metric: "仅服务端",
          },
          {
            detail: "本地和自带密钥界面会在端到端测试通过后恢复。",
            label: "发布闸门",
            metric: "已推迟",
          },
        ],
