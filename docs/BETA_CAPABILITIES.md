...
| PostgreSQL persistence and idempotency       | In scope                       | Durable ticket redemption, abuse counters, and billable-call coordination exist. PostgreSQL concurrency and upgrade tests remain release gates.                                                       |
| Operator Console                             | In scope behind authentication | Every page is proxy-protected; runtime reads and Server Action mutations recheck an eight-hour operator session or automation bearer token. Static sample fallback is not authoritative runtime data. |
